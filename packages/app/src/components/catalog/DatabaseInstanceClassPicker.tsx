...
class Filter implements EntityFilter {
    ...
    filterEntity(entity: Entity): boolean {
        const specValue = entity.spec?.instanceClass;
    ...
    getCatalogFilters(): Record<string, string | string[]> {
        return { 'spec.instanceClass': this.values };
    }
}


export const DatabaseInstanceClassPicker = () => {
    const {
        filters,
        updateFilters,
        queryParameters: { instanceClass: instanceClassParams },
    } = useEntityList<Filters>();

    const queryParam = useMemo(
        () => [instanceClassParams].flat().filter(Boolean) as string[],
        [instanceClassParams],
    );

    const [selectedValue, setSelectedValue] = useState(
        queryParam.length ? queryParam : filters?.instanceClass?.values ?? [],
    );
    ...
    const {
        ...
        loading,
        error,
    } = useAsync(async () => catalogClient.getEntityFacets({ facets: ["spec.instanceClass"] }), []);
    ...
    useEffect(() => {
        updateFilters({
            instanceClass:
                selectedValue?.length && facets?.facets["spec.instanceClass"].length
