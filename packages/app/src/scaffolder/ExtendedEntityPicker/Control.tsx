 ...
 * @public
 */
export const EntityPickerWithRepo = (props: EntityPickerProps) => {
  const {
    onChange,
...
const showPillarOnly = uiSchema['ui:options']?.showPillarOnly || false;
  ...
  const getLabelText = (entity: Entity|string) => {
    ...
    const parts = [];
    if (tryShowDisplayName) {
      parts.push((entity.spec?.profile as any)?.displayName || entity.metadata.name);
    } else {
      parts.push(humanizeEntityRef(entity))
    }
  ...
  const getLabel = useCallback(
    (ref: string, entity: Entity | undefined) => {
      try {
        return getLabelText(entity as Entity);
      } catch (err) {
        return ref;
      }
    },
  ...
  return (
    ...
    >
      <Autocomplete
        ...
        onChange={onSelect}
        options={entities || []}
        getOptionLabel={(option: any) => getLabelText(option)}
        autoSelect
